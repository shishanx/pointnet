...
parser.add_argument('--file_name', default='epoch_190.ckpt', help='Model checkpoint path')
FLAGS = parser.parse_args()


# DEFAULT SETTINGS
file_name = FLAGS.file_name # os.path.join(BASE_DIR, './pretrained_model/model.ckpt')
...
def predict():
    ...
    with tf.Session(config=config) as sess:
        ...
        printout(flog, 'Loading model train_results/trained_models/%s' % file_name)
        saver.restore(sess, 'train_results/trained_models/%s' % file_name)
